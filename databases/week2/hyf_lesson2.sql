...
SELECT user.name AS user_name, task.title AS task_name, status.name as status_name
FROM user, task, status, user_task
...
SELECT task.title as task_title, task.id as task_id, user.name as user_name, task.created
...
SELECT COUNT(task.id) AS Count_Tasks, month(created) AS Created_Month
FROM task
